...
int ctNodes(struct Node*root)
{
    if(root==NULL)
    return 0;
    
    return 1+ctNodes(root->left) + ctNodes(root->right);
}
bool isCBT(struct Node*root, int i, int ct)
{
    if(root==NULL)
    return true;
    ...
    bool left = isCBT(root->left,2*i,ct);
    bool right = isCBT(root->right,2*i+1,ct);
...
bool isHeap(struct Node* root) {
    bool a=isCBT(root,1,ctNodes(root));
    bool b=isMaxHeap(root);
    return a && b;
}
